...
AI 모델 서버와 모델 설정은 사용하지 않는다
영상 처리 산출물에 서버 rules 필터를 적용해 결과 화면으로 전달한다
