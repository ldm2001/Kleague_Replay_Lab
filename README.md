...
접촉과 원심 및 재개 방법은 검증 전이므로 사실값을 만들지 않는다. 계약은 [운영 연결](docs/design/운영판정연결.md), 실행은 [Worker](apps/video-worker/README.md)를 따른다

2026년 9월 13일 승인된 첫 시청각 연결은 전체 원본의 고정 DSP 음향 관측과 소리 포함 증거 클립 및 원본 시간축 대응이다
기본 경로는 `video-local-observers-av-v1`이며 음향은 비공개 관측으로만 보존한다. 발화 전사·화자·파울 판단은 지원하지 않는다
보존·동기화 시험과 실제 인식 정확도를 구분한 결과는 [영상·음향 연결 점검](docs/analysis/영상음향연결점검.md)에 기록한다

2026년 9월 21일 추가한 서버 자동 평가 경로는 사건별 완료·보류를 별도 저장하고 검증된 완료 결과만 공개한다
현재 접촉 강도와 경기 맥락의 검증된 사실 생산자는 없으므로 실제 자동 밀기 판정을 활성화한 것은 아니다
적용에는 `0020_automatic_reviews`와 서버·Worker `video-observations-v5` 동시 갱신이 필요하다. 새 매체 증거는 체크섬·revision 결합 최초 PUT으로 보존한다

현재 자동 분석은 등록된 원본의 득점 방송 표시를 추출하고 K리그1·2의 시즌별 요강에 따른 VAR 적용 범주를 별도 완료 결과로 제공한다
이 범위 분석은 실제 득점 인정과 VAR 실시 및 오심 판정이 아니며 상세 검증과 제한은 [K리그 연결 점검](docs/analysis/K리그연결점검.md)을 따른다
> 아래 세부 설계 중 영상 모델과 사용자 보정에 관한 내용은 과거 설계 기록이며 [자동 처리 설계](docs/design/자동처리.md)를 우선한다
...
→ 낮은 신뢰도 후보 포함
→ 핵심 프레임과 짧은 증거 클립 생성
→ 파이프라인 산출물과 적용 가능한 규정 조건 필터링
→ 근거 부족과 적용 판본 미확정 상태 표시
→ 근거와 한계가 포함된 결과 표시
```
...
| 파이프라인 결과 | 화면 변화 후보와 원본 프레임과 짧은 증거 클립 표시 |
...
| 영상 처리 | Python Worker와 FFmpeg와 ffprobe와 OpenCV |
...
외부 모델 서버는 사용하지 않는다. 전용 Python 환경과 고정된 세 모델 자산을 먼저 준비한다
준비 방법과 운영자 장치 설정은 [Worker 실행](apps/video-worker/README.md)을 따른다
영상 산출물에 서버 근거 검증과 rules 필터를 적용하며 완료된 규정 평가만 화면으로 전달한다
...
현재 제품 경로는 영상 업로드 → 자동 파이프라인 → rules 필터 → 결과 열람이다
대회와 시즌 및 분석 임계값과 사실 입력을 사용자에게 요구하지 않는다
확인되지 않은 경기 정보와 접촉 사실은 확인 불가로 남긴다
아래의 기존 모델과 사실 보정 설계는 과거 기록이며 현재 자동 처리 경로의 요구사항이 아니다
