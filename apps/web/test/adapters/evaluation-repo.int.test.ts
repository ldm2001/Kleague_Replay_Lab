...
describeDatabase("PostgreSQL evaluation repository", () => {
  ...
  it("stores facts with shot links and a cited IFAB and K League decision", async () => {
    ...
    const decision = await repository.save({ anonymousSessionId: sessionId, analysisId, candidateId, factRevisionId: saved.factRevisionId, ruleVersionId: context.value.ruleVersionDbId, facts, evaluation: judged.value, ruleEngineVersion: "rule-engine-v1", evaluationSchemaVersion: 1, now: NOW });
