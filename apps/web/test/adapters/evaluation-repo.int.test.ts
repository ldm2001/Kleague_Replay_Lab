...
import { evaluationRepo } from "@replay/adapters";
import { evaluate } from "@replay/application";
...
describeDatabase("PostgreSQL evaluation repository", () => {
  ...
  it("stores facts with shot links and a cited IFAB and K League decision", async () => {
    ...
    const run = evaluate({ rule: ruleSet, push: pushResult, variable: varResult, hash: async (input) => Uint8Array.from(digest("sha256").update(input).digest()) });
  ...
  it("keeps a newer failed analysis state when a stale evaluation finishes", async () => {
    ...
    await expect(repository.save({ anonymousSessionId: sessionId, analysisId, candidateId, factRevisionId: factId, ruleVersionId: context.value.ruleVersionDbId, analysisStateVersion: context.value.analysisStateVersion, facts, evaluation: judged.value, ruleEngineVersion: "rule-engine-v1", evaluationSchemaVersion: 1, now: NOW })).resolves.toMatchObject({ kind: "CREATED" });
