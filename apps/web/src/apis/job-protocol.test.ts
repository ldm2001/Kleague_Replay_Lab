...
describe.each(routes)("worker protocol at %s", (_name, invoke) => {
  it.each([undefined, "", "video-baseline-v1", "video-observations-v2", "video-observations-v3", "video-observations-v4", "video-observations-v99"])(
    ...
    async (protocol) => {
      ...
      expect(await response.json()).toEqual({
        kind: "UNSUPPORTED_WORKER_PROTOCOL", requiredProtocol: "video-observations-v5",
      });
