...
export const analysisPerceptionRuns = pgTable(
  ...
  (table) => [
    ...
    check("analysis_perception_runs_version_pair_check", sql`(${table.schemaVersion} = 'perception-run-v1' and ${table.pipelineVersion} = 'video-local-observers-v1') or (${table.schemaVersion} = 'perception-run-v2' and ${table.pipelineVersion} = 'video-local-observers-av-v1')`),
