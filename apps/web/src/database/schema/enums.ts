...
export const idempotencyOperation = pgEnum(
    ...
    values(["CREATE_ANALYSIS", "PATCH_FACTS"])
);
// 검토 대상 상황
...
// 판정 일치 여부
export const decisionMatch = pgEnum(
    "decision_match",
    values(["MATCH", "MISMATCH", "UNDETERMINED"])
);
// 접촉 강도
...
// 비디오 판독 검토 절차
export const varReviewProcedure = pgEnum(
    "var_review_procedure",
    values(["OFR", "VAR_ONLY", "NONE"])
);
// 판정 보류 사유
...
// 공식 판정 상태
export const officialVerdictStatus = pgEnum(
    "official_verdict_status",
    values(["EXTERNAL_OPINION"])
);
