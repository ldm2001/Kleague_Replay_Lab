...
export const observationData = (value: unknown): value is SceneObservation => {
    ...
    return (
        ...
        item.model.length > 0 &&
        item.model.length <= 128 &&
        typeof item.category === "string" &&
        ["PUSHING", "OTHER", "UNKNOWN"].includes(item.category) &&
        typeof item.contact === "string" &&
        ["YES", "NO", "UNKNOWN"].includes(item.contact) &&
        typeof item.displacement === "string" &&
        ["clear", "none", "possible", "uncertain"].includes(item.displacement) &&
        typeof item.camera === "string" &&
        ["LOW", "MEDIUM", "HIGH"].includes(item.camera) &&
        typeof item.summary === "string" &&
        item.summary.trim().length > 0 &&
        item.summary.length <= 1200 &&
        Array.isArray(item.timestamps) &&
        item.timestamps.length > 0 &&
        item.timestamps.length <= 8 &&
