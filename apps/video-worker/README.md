...
`npm run test:video`는 FFmpeg와 ffprobe의 실제 판본 및 빌드 설정을 시험 로그에 남긴다
CI도 웹 시험 전에 같은 도구 정보를 기록하며 특정 종료 코드나 과거 인코딩 결과를 정답으로 고정하지 않는다
음향 관측은 오류 수준 디코더 출력이 있으면 종료 코드가 영이어도 `FAILED`로 처리한다
증거 클립은 인코딩 오류 출력과 실제 출력 음향 디코딩을 확인한 뒤에만 `PRESERVED`로 기록한다
음향 실패 뒤 영상 단독 재시도가 성공하면 `OMITTED_DECODE_FAILED`를 보존하며 영상 재시도도 실패하면 클립 자체를 거부한다
`AUDIO_OUTPUT_DECODE_FAILED_VIDEO_RETRY_SUCCEEDED`는 출력 음향 검증 실패 후 영상 재시도 성공을 뜻한다
요청 구간의 음향 표본 부재와 출력 부재 및 디코딩 오류와 시간 초과 등을 포함할 수 있으므로 원본 음향 손상의 확정 근거로 해석하지 않는다
현재 `audioOutput()`은 불리언 계약이며 상세 원인 분리는 별도 보완 대상이다
