...
- `infra/` 로컬 인프라 설정
- `experiments/perception/` 승인된 비생성형 검출·추적의 격리 실증이며 운영 경로가 아님
...
- Gemma와 Ollama 및 생성형 모델과 승인 목록 밖 모델을 파이프라인에 추가하지 않는다
- 흐름은 `영상 → 파이프라인 → rules 필터 → 프론트 결과`다
- 화면 변화 점수와 후보 존재를 접촉이나 파울 판정으로 바꾸지 않는다
- 원시 변화 후보는 진단에 보존하고 인식 사건과 규정 판단 상태를 구분한다
- 자동 분석의 최종 화면에는 규정 평가가 완료된 결과만 표시하고 보류와 인식만 된 장면은 내부에 보존한다
- VAR 적용 범주 분석의 완료와 전체 파울 판정의 완료를 별개로 관리한다
...
직접 작성한 코드는 짧은 명사형 이름·함수별 한 줄 주석·단계당 스페이스 4개 들여쓰기·함수 사이 빈 줄 한 줄·import 사이 빈 줄 없음을 따른다. 필수 문법·외부 인터페이스·데이터 계약·문자열 내용은 보존한다. 범위와 예외는 [명명 정리](docs/plans/명명정리.md)를 따른다.
긴 조건·인자·타입·JSX는 [코드 가독성](docs/design/코드가독성.md)에 따라 구문 단위로 나눈다. 공백 수만 일괄 변경하는 것으로 가독성 정리를 끝내지 않는다.
사람이 읽는 주석은 한글 명사형으로 작성하며 생성·읽음·확인·반환처럼 마무리한다. 주석 내용의 마침표·쉼표·큰따옴표는 제외하되 도구 지시문과 언어 문법 기호는 보존한다.
...
9월 21일 자동 평가 저장·공개 연결은 [전체 영상 자동 평가](docs/design/전체영상자동평가연결.md)를 따른다. 연결 시험 통과와 실제 인식 사실의 검증·활성화는 별개다
후속 일곱 요소의 관측·평가 분리와 질문별 완료는 [사건별 출력 명세](docs/design/사건별출력명세.md)를 따른다. 기존 이력과 새 계약을 구분하고 운영 인식 활성화로 간주하지 않는다
